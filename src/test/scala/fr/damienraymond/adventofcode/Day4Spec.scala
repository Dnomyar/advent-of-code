...
class Day4Spec extends AnyFlatSpec with Matchers {
  ...
  it should "return the sum unmarked numbers 1" in {
    val Some(grid) = BingoGrid.fromString(
      """1 2
        |3 4""".stripMargin)
    Day4.scorePart1(Vector(grid), List(1,2,3,4)) should contain ((3 + 4) * 2)
  }
  ...
  it should "return the sum unmarked numbers 2" in {
    val Some(grid) = BingoGrid.fromString(
      """1 2
        |3 4""".stripMargin)
    Day4.scorePart1(Vector(grid), List(1,3,4,2)) should contain ((2 + 4) * 3)
  }
  ...
  it should "empty if the grid did not won" in {
    val Some(grid) = BingoGrid.fromString(
      """1 2
        |3 4""".stripMargin)
    Day4.scorePart1(Vector(grid), List.empty) should be (empty)
    Day4.scorePart1(Vector(grid), List(5,6,7,8,9)) should be (empty)
  }
  ...
  it should "return the sum unmarked numbers for the grid that won" in {
    ...
    val Some(grid2) = BingoGrid.fromString(
      """5 6
        |7 8""".stripMargin)
    Day4.scorePart1(Vector(grid1, grid2), List(1,6,7,3,4,9)) should contain ((2+4)*3)
  }
  ...
  it should "parse and execute the program for the example for part 2" in {
    val input = """7,4,9,5,11,17,23,2,0,14,21,24,10,16,13,6,15,25,12,22,18,20,8,19,3,26,1
                  |
                  |22 13 17 11  0
                  | 8  2 23  4 24
    ...
    Day4.parseAndProgramPart2(input) should contain (1924)
  }
  it should "return return the actual result for the all input part2" in {
    val res = Day4.day4Input.use(lines =>
      IO(Day4.parseAndProgramPart2(lines.mkString("\n")))
    ).unsafeRunSync()
